...
exports.view_meals_by_day = function(req, res) {
  var token = req.headers.authorization.substring(4);
  var userInfo = jwt.decode(req.headers.authorization.substring(4));
  const day = moment(req.params.day + ' 12:00 am', 'MM-DD-YYYY HH:mm a').utcOffset(req.params.offset)
  const startDay = new Date(day.clone().utcOffset(req.params.offset).format('YYYY-MM-DD HH:mm'))
  const endDay = new Date(day.clone().endOf('day').utcOffset(req.params.offset).format('YYYY-MM-DD HH:mm'))
  Meal.find({mealUser: ObjectId(userInfo._id), mealDate: {"$gte": startDay, "$lt": endDay}}).sort({ 'mealDate': 1 })
    .exec(function (err, results) {
      ...
      res.json({
        ...
        'day': req.params.day,
        'meals': meals,
        'clientoffset': req.params.offset,
