...
exports.view_meals_by_day = function(req, res) {
  var token = req.headers.authorization.substring(4);
  var userInfo = jwt.decode(req.headers.authorization.substring(4));
  const day = moment(req.params.day + ' 12:00 am', 'MM-DD-YYYY HH:mm a').utcOffset(req.params.offset, true)
  const startDay = new Date(day.clone().utcOffset(req.params.offset, true).format('YYYY-MM-DD HH:mm'))
  const endDay = new Date(day.clone().endOf('day').utcOffset(req.params.offset, true).format('YYYY-MM-DD HH:mm'))
